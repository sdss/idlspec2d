...
pro rm_sky_spec, outdir=outdir, run1d=run1d1, doplot=doplot, spectradir=spectradir, $
...
CPU, TPOOL_NTHREADS = 1  
 ...
 if (NOT keyword_set(outdir)) then outdir="outdir"
  ...
  logfile = djs_filepath('SKYRMSpec-' + run2d + '.log', root_dir=outdir)
...
;  program=program[nt]
  ...
  finalvalues=replicate(create_struct('MAG',fltarr(5)),NexpF)
  values_t=replicate(create_struct('SKYMAG',fltarr(5)),NexpF)
  finalvalues=struct_addtags(finalvalues,values_t)
  values_t=replicate(create_struct('MAGERR',fltarr(5)),NexpF)
  finalvalues=struct_addtags(finalvalues,values_t)
  values_t=replicate(create_struct('SKYMAGERR',fltarr(5)),NexpF)
  finalvalues=struct_addtags(finalvalues,values_t)
  ...
  finalvalues=struct_addtags(finalvalues,values_t)
  magvec=finalvalues.mag
  skymagvec=finalvalues.skymag  
  magerrvec=finalvalues.magerr
  skymagerrvec=finalvalues.skymagerr
  ...
  ct=ulong64(0)
  spawn,'mkdir -p '+outdir
  outfile=outdir+'/spSkyRM_spec-'+run2d+'.fits'
    ...
    if spec eq 1 then begin
      ...
      if not keyword_set(lite) then begin
         ;for i=0, nexp-1 do begin
           ...
           flux=single.flux
           ivar=single.ivar
           lam=10^(single.loglam)
           skyfkux=single.sky
           snimg = flux ;* sqrt(ivar)
           
           flux_mag=rm_spec2mag(lam, flux, ivar, synmag_err=flux_mag_err)
           skyflux_mag=rm_spec2mag(lam, skyfkux, ivar, synmag_err=skyflux_mag_err)
           
           magvec[*,ct]=flux_mag
           skymagvec[*,ct]=skyflux_mag
           magerrvec[*,ct]=flux_mag_err
           skymagerrvec[*,ct]=skyflux_mag_err
  ...
  finalvalues.mag=magvec
  finalvalues.skymag=skymagvec  
  finalvalues.magerr=magerrvec
  finalvalues.skymagerr=skymagerrvec
