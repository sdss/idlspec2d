...
pro snr_frame, outdir=outdir, run1d=run1d1, doplot=doplot, spectradir=spectradir, $
...
CPU, TPOOL_NTHREADS = 1  
  ...
  finalvalues=replicate(create_struct('SNR_g',0.0),NexpF)
  values_t=replicate(create_struct('SNR_i',0.0),NexpF)
  ...
  finalvalues=struct_addtags(finalvalues,values_t)
  snvec_g=finalvalues.snr_g
  snvec_i=finalvalues.snr_i
    ...
    if spec eq 1 then begin
      ...
      id_str2=id_str+'-'+expnum
           ...
           if (nwave GT filtsz) then $
             sntemp = djs_median(snimg[gwave[ig]], $
           ...
           sntemp=0
           if (nwave GT filtsz) then $
             sntemp = djs_median(snimg[iwave[ig]], $
             width=filtsz, boundary='reflect')
           snr = djs_mean(sntemp)
           snvec_i[ct]=snr
  ...
  finalvalues.snr_g=snvec_g
  finalvalues.snr_i=snvec_i
