...
;
; PURPOSE:
;   Solve the linear set of equations Ax=b using LA_SVD
;
; CALLING SEQUENCE:
;   chi2 = computechi2_LA( bvec, sqivar, amatrix, $
...
;-
;------------------------------------------------------------------------------
function computechi2_la, objflux, sqivar, starflux, $
 ...
 status=status
   ...
   ; Use SVD_LA to invert the matrix
   if (nstar EQ 1) then begin
      ; The last term below is to protect against divide-by-zero
      ; in the degenerate case.
      mmi = 1.0 / (mm + (mm EQ 0))
   endif else begin
      LA_SVD, mm, ww, uu, vv, /double, status=status
      if status gt 0 then begin
          splog, "ERROR: Computechi2: SVDC: svdcmp_d did not converge."
          return, -1
      endif
      mmi = 0 * vv
      ; The last term below is to protect against divide-by-zero
