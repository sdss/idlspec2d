...
;                default to '/data/spectro/plugMapM'
;   guidedir   - Directory containing guider images;
;                default to '/data/spectro/guider'
;
; OUTPUTS:
;
; OPTIONAL OUTPUTS:
...
;------------------------------------------------------------------------------
function sdssguide_read_image, thisfile
   ...
   case dims[0] of
      ...
      512: begin
         ...
         image = rotate(image, 4) ; new camera binned 2x2
         end
      1024: begin
         camera = 'new'
         camoffsets = [427., 441.]
         ...
         rdnoise = 8.1 ; electrons
         gain = 1.4 ; electrons/ADU
         image = rotate(rebin(image, dims/2), 4) ; new camera unbinned
         end
   endcase
...
;------------------------------------------------------------------------------
pro sdssguide_centroid_stars, img, guideobj

   nfiber = n_elements(guideobj)

   for i=0L, nfiber-1L do begin
      if (guideobj[i].enable AND guideobj[i].goodfiber) then begin
         ...
         parinfo[2].value = 6.
         parinfo[2].limits = [2., 10.] ; limits of seeing in pixels???
      ...
      if (keyword_set(fitval)) then begin
...
pro sdssguide_compute_teloffset, plughdr, img, guideobj, guideiter, pluginfo
   ...
   berr = [guideobj.dra_err, guideobj.ddec_err] * guideobj.goodfiber
...
pro sdssguide_display, img, guideobj
   ...
   for i=0L, nfiber-1L do begin
      if (guideobj[i].enable EQ 0) then text = 'DISABLE' $
       else if (guideobj[i].goodfiber EQ 0) then text = 'FAINT SKY' $
       else text = ''
      ...
      disable_string = guideobj[i].enable ? '' : ' DISABLE'
      atvxyouts, guideobj[i].xfiber-guideobj[i].rfiber, $
       guideobj[i].yfiber+guideobj[i].rfiber, $
       strtrim(guideobj[i].fiber,2), $
       color='red', charsize=2, align=1
      if (keyword_set(text)) then $
       atvxyouts, guideobj[i].xfiber, $
        guideobj[i].yfiber-guideobj[i].rfiber-10, $
        text, color='red', charsize=1.5, align=0.5
   ...
   atvxyouts, 0, -25, fileandpath(img.filename), charsize=2, color='red'
...
;guidedir1 ='/Users/schlegel/rawdata/51820/guider' ; ???
   ...
   if (keyword_set(plugdir1)) then plugdir = plugdir1 $
    else plugdir = '/data/spectro/plugMapM'
   if (keyword_set(guidedir1)) then guidedir = guidedir1 $
    else guidedir = '/data/spectro/guider'
   quiet = !quiet
   !quiet = 1
   except = !except
   !except = 0
   ...
   while (cc NE 'Q') do begin
      ...
      files = file_search(djs_filepath('g*.fits*', root_dir=guidedir))
      finfo = file_info(files)
...
;thisfile ='test-cart4.fits'
      if (thisfile NE lastfile) then begin
         ...
         print, ' '
         print, 'File = '+thisfile
         img = sdssguide_read_image(thisfile)
         if (sdssguide_is_valid(img)) then begin
            ...
            sdssguide_centroid_fibers, cartinfo, img, guideobj, minval=0
            sdssguide_centroid_stars, img, guideobj
