...
;   Added by vivek- Same as quickextract, but include
;   modifications to the ELG program. Major modifications are :
;   1) Blue camera SN2 values are hardwired to be 4.0 for each exposure
;   2) Red camera meanSN2 values are computed in the sky free z-band wavelengths
;   3) A scale factor is added to match the ELG depth to a typical normal eBOSS plate depth 
;
; CALLING SEQUENCE:
;   rstruct = quickextract_elg(tsetfile, wsetfile, fflatfile, rawfile, outsci, $
...
;   8-Sep-2016  quickextract_elg Vivek M.
;------------------------------------------------------------------------------
function quickextract_elg, tsetfile, wsetfile, fflatfile, rawfile, outsci, $
...
;   endif
   ...
   endif else begin ; no i band for ELG plates. The SN2 measurements are done at z-band -vivek
	icolor = 4
	snfilter = 'z'
	wrange = [8050,8250, 8550,8750, 9000,9300 ]; coverage of z-band, wavelengths free of sky lines - vivek
   endelse
   ...
   if (colorband EQ 'b') then begin
      ...
      iwave = where((logwave[*,ifib] GT alog10(wrange[0]) $
                AND logwave[*,ifib] LT alog10(wrange[1])) $
                OR (logwave[*,ifib] GT alog10(wrange[2]) $
                AND logwave[*,ifib] LT alog10(wrange[3])) $
                OR (logwave[*,ifib] GT alog10(wrange[4]) $
                AND logwave[*,ifib] LT alog10(wrange[5])) $
                AND objsubivar[*,ifib] GT 0, nwave)
endelse
...
;	print,'--quickextract-fitsn (median mag) ',median( plugsort[iobj].mag[icolor]),median(meansn[iobj])
...
;	print,'#########################     CHECK       ##############################'
   ...
   if (colorband EQ 'b') then begin
	...
	sn2=sn2*2.11; multiplying with the scale factor to match the depth of an eBOSS plate - vivek
   endelse
   endif else begin
      sn2 = 0
   endelse
