...
pro writespectra, objhdr, flux, fluxivar, plugsort, wset, $
 filebase=filebase, individual=individual
...
;
     if (keyword_set(fluxivar)) then mwrfits, fluxivar, outname
     if (keyword_set(plugsort)) then mwrfits, plugsort, outname
     if (keyword_set(wset))     then mwrfits, wset,     outname
