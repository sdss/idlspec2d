...
;-
;------------------------------------------------------------------------------
pro uubatchpbs_directives, pbs_batch_lun=pbs_batch_lun, slurm=slurm, pbs_batch=pbs_batch
...
; Do not use spPlancomb files that only have a subset of the MJDs !!!???
   ...
   if keyword_set(pbs_nodes) then begin
     ...
     for pbs_node = 0, pbs_nodes-1 do begin
       ...
       uubatchpbs_directives, pbs_batch_lun=pbs_node_lun[pbs_node], slurm=slurm
       if keyword_set(pbs_ppn) then begin
     ...
     if keyword_set(pbs_batch) then begin
        ...
        uubatchpbs_directives, pbs_batch_lun=pbs_batch_lun, slurm=slurm, /pbs_batch
        close, pbs_batch_lun
