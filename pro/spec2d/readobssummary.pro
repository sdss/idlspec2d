...
;               reddening value for the entire field as found in the
...
;   16-Jan-2019  Renamed as readobssummary by H. J. Ibarra-Medel
;-  
;------------------------------------------------------------------------------
function readobssum_sort, plugmap, fibermask=fibermask

   qobj = strmatch(plugmap.fibertype,'BOSS')
   qobjapo = strmatch(plugmap.fibertype,'APOGEE')
   ...
   if (ngood EQ 0) then $
    message, 'No fibers found in obsSummary file!'
...
;------------------------------------------------------------------------------
function readobssummary, obssfile, spectrographid, plugdir=plugdir, $
 apotags=apotags, deredden=deredden, exptime=exptime, calibobj=calibobj, $
 hdr=hdr, fibermask=fibermask, n_names=n_names, _EXTRA=KeywordsForPhoto
   if n_elements(obssfile) GT 1 then begin
      nnames=1
      ;sconfig = strarr(n_elements(config))
      for i=0, n_elements(obssfile)-1 do begin
        if i EQ 0 then begin
          plugmap = readobssummary(obssfile[i], spectrographid, plugdir=plugdir, $
            apotags=apotags, deredden=deredden, exptime=exptime, calibobj=calibobj, $
            hdr=hdr, fibermask=fibermask, n_names=nnames, _EXTRA=KeywordsForPhoto)
            hdr=['cut',hdr]
            fibermask=[-100,fibermask]
        endif
        if i GT 0 then begin
          fibermask1=0
          plugmap1 = readobssummary(obssfile[i], spectrographid, plugdir=plugdir, $
            apotags=apotags, deredden=deredden, exptime=exptime, calibobj=calibobj, $
            hdr=hdr1, fibermask=fibermask1, n_names=nnames, _EXTRA=KeywordsForPhoto)
          plugmap = [plugmap ,plugmap1]
          hdr= [hdr,'cut',hdr1]
          fibermask = [fibermask,-100, fibermask1]
        endif
      endfor
      hdr= [hdr,'cut']
   ...
   ;----------
   ; Read the file
   thisfile = (findfile(djs_filepath(obssfile, root_dir=plugdir), $
    count=ct))[0]
   if (ct NE 1) then begin
      splog, 'WARNING: Cannot find obsSummary file ' + obssfile
      return, 0
   endif

   yanny_read, thisfile, pstruct, hdr=hdr, stnames=stnames, /anonymous
   if (NOT keyword_set(pstruct)) then begin
      splog, 'WARNING: Invalid obsSummary file ' + thisfile
      return, 0
   endif
   plugmap = *pstruct[(where(stnames EQ 'FIBERMAP'))[0]]
   ...
   plugmap = readobssum_sort(plugmap, fibermask=fibermask)
   ...
   if (keyword_set(platelist_dir)) then begin
      ...
      if (ct GT 0) then begin
         ...
         tmp = strsplit(file_basename(pobssfile), '-.', /extract)
...
;            endfor
   ...
   ;endif
   if (keyword_set(n_names)) EQ 0 then begin
       hdr= ['cut',hdr,'cut']
