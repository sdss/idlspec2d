...
;     speclog file: guiderMon-mmmmm.par (where mmm is MJD) 
;
...
pro spadd_guiderinfo, hdr
   ...
   if (keyword_set(pdata)) then begin
      for i=0, N_elements(pdata)-1 do $
        if (stnames[i] EQ 'GUIDEOBJ') then guidermon = *pdata[i]
        ...
        if found[0] NE -1 then begin
          ...
          if ct2 GT 8 then begin   ; at least 8 fibers?
          ...
          endif else splog, 'Warning: No inclusive times'
        endif else splog, 'Warning: Could not find plateId'

      yanny_free, pdata
   endif else begin
      splog, 'WARNING: Empty or invalid guiderMon file '+guidermonfile
   endelse
