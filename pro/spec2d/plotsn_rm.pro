...
;   Modified plotsn with a hardcoded scale factor multiplied to SN2 values.
;   The scale factor is multiplied to the sn2 values to enable deeper
;   exposures 
;   This is mainly written  for the special case of Reverberation 
;   Mapping plates. Use plotsn otherwise.
;
; CALLING SEQUENCE:
;   plotsn_rm, snvec, plugmap, [ filter=, plotmag=, plottitle=, snmin=, $
...
;
; REVISION HISTORY:
;   23-Jan-2018  Added scale factor to change depth of RM plates by Vivek 
...
;------------------------------------------------------------------------------
pro plotsn_rm, snvec1, plugmap1, filter=filter1, plotmag=plotmag1, snmin=snmin1, $
...
;	print,'--plotsn-fitsn (mag) s1 ', snvec[iband,s1]
         ...
         ; hardcoded scalefactor multiplied to SN2 -- vivek
          snplate[0,iband] = 0.8*sn2
