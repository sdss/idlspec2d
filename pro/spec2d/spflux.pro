...
pro spflux, sciname
   ...
   for iexp=0, nall-1 do begin
      iblue = (where(camcolor EQ 'b' AND exposure EQ allexpnum[iexp]))[0]
      ired = (where(camcolor EQ 'r' AND exposure EQ allexpnum[iexp]))[0]
   ...
   ibcalib = (where(camcolor EQ 'b' AND exposure EQ bestexpnum))[0]
   ircalib = (where(camcolor EQ 'r' AND exposure EQ bestexpnum))[0]
   ...
   bcalibfile = $
    'spFluxcalib-' + platestr + '-' + mjdstr + '-' + cameras[ibcalib] + '.fits'
   rcalibfile = $
    'spFluxcalib-' + platestr + '-' + mjdstr + '-' + cameras[ircalib] + '.fits'

   myfluxcalib, sciname[ibcalib], sciname[ircalib], bcalibset, rcalibset

   mwrfits, bcalibset, bcalibfile, /create
   mwrfits, rcalibset, rcalibfile, /create
   ...
   for iexp=0, nall-1 do begin
      ...
      myfluxcorr, sciname[ibcalib], sciname[ircalib], bcalibset, rcalibset, $
       bluefile, redfile, corrfile
   endfor
