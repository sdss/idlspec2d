...
;   rm_info    - If set, overlay the release and i_mag info for the RM targets
;   old_spec   - If set, overplot the earlier SDSS/BOSS spectrum in cyan
;   masksky    - /masksky to overplot pixels near bright skylines
;   calibdir   - calibdir='recalib/' to use the recalibrated spectrum
...
;   05-Jan-2014  Modified for SDSS-RM purposes by Y. Shen, Carnegie
;-
;------------------------------------------------------------------------------
...
;------------------------------------------------------------------------------
pro rm_plotspec1, plate, fiberid, mjd=mjd, znum=znum, zmanual=zmanual, $
 nsmooth=nsmooth1, zline=q_zline, nosyn=nosyn, noerr=noerr, sky=sky, $
 ormask=ormask, andmask=andmask, masksky=masksky, $
 ...
 rm_info = rm_info, rm_fiber=rm_fiber, old_spec=old_spec,calibdir=calibdir

   common plotspec_data, skymask
   ...
   rm_readspec, plate, fiberid, mjd=mjd, znum=znum, flux=objflux, $
    wave=wave, plug=plug, zans=zans, _EXTRA=Extra, /silent, calibdir=calibdir
   ...
   if (NOT keyword_set(noerr)) then $
    rm_readspec, plate, fiberid, mjd=mjd, flerr=objerr, _EXTRA=Extra, /silent,calibdir=calibdir
   if (keyword_set(zmanual)) then begin
      rm_readspec, plate, fiberid, mjd=mjd, invvar=objivar, loglam=loglam, $
       objhdr=hdr, _EXTRA=Extra, /silent, calibdir=calibdir
      ...
      if (NOT keyword_set(nosyn)) then $
       rm_readspec, plate, fiberid, mjd=mjd, znum=znum, synflux=synflux, $
        _EXTRA=Extra, /silent,calibdir=calibdir
   endelse
   if (keyword_set(sky)) then $
    rm_readspec, plate, fiberid, mjd=mjd, sky=sky, _EXTRA=Extra, /silent,calibdir=calibdir
   if (keyword_set(ormask)) then $
    rm_readspec, plate, fiberid, mjd=mjd, ormask=ormask, _EXTRA=Extra, /silent,calibdir=calibdir
   if (keyword_set(andmask)) then $
    rm_readspec, plate, fiberid, mjd=mjd, andmask=andmask, _EXTRA=Extra, /silent,calibdir=calibdir
   if (keyword_set(zans) AND keyword_set(q_zline)) then $
    rm_readspec, plate, fiberid, mjd=mjd, zline=zline, lineflux=lineflux, $
     _EXTRA=Extra, /silent,calibdir=calibdir
   ...
   if keyword_set(rm_info) then begin
      xpos1 = 0.9 * !x.window[0] + 0.7 * !x.window[1]
      dxpos1 = 0.05 * (!x.window[0] - !x.window[1])
      dypos1 = 0.05 * (!y.window[0] - !y.window[1])
      ypos1 = !y.window[1] + 1.5 * dypos1
      ; find the epoch (i.e., the corresponding plugging)
      ind_epoch = where( (rm_fiber.plate)[*,0] eq plate and $
	(rm_fiber.mjd)[*,0] eq mjd)
      ind_obj = where(rm_fiber.fiberid[ind_epoch,*] eq fiberid)
      ; If we are looking at the coadded plate (all epochs) then index is just fiberid-1
      if ind_epoch[0] eq -1 or ind_obj[0] eq -1 then ind_obj=fiberid-1
      sxyouts, xpos1 + 2.*dxpos1, ypos1 + dypos1, $
       strtrim(rm_fiber[ind_obj].release), /norm, $
       charsize=csize, color=textcolor, _EXTRA=KeywordsForSplot
      ypos1 = ypos1 + dypos1
      sxyouts, xpos1, ypos1, 'i=' + $
       string(rm_fiber[ind_obj].psfmag[3],format='(f6.3)'), /norm, $
       charsize=csize, color=textcolor, _EXTRA=KeywordsForSplot

      ; overplot the existing spectrum (DR7 or BOSS)
      if keyword_set(old_spec) then begin
        old_specfile = getenv('IDLRM_DIR') + '/etc/early_spec/' + $
         string(rm_fiber[ind_obj].dr_plate,format='(i4.4)') + '-' + $
         string(rm_fiber[ind_obj].dr_fiberid,format='(i4.4)') + '-' + $
         string(rm_fiber[ind_obj].dr_mjd, format='(i5.5)') + '.dat'
        ;print, old_specfile
        if file_test(old_specfile) eq 1 then begin
           readcol, old_specfile, format='d,d',wave_oldspec,flux_oldspec,/silent
           soplot, wave_oldspec, smooth(flux_oldspec, nsmooth),color='magenta'
           sxyouts, xpos1, ypos1 + dypos1, 'MJD='+string(rm_fiber[ind_obj].dr_mjd $
               , format='(i5.5)'), color='magenta', charsize=csize,/norm
        endif
      endif
   endif
   ...
   if keyword_Set(masksky) then begin
      if n_elements(skymask) eq 0 then skymask = rm_skymask(wave)
      if keyword_set(restframe) then skymask = rm_skymask(wave*(1.+zans.z))
      ind_sky = where(skymask eq 0)
      if ind_sky[0] ne -1 then soplot, wave[ind_sky], objflux[ind_sky],psym=4,color='cyan'
   endif
...
;------------------------------------------------------------------------------
pro rm_plotspec, plate, fiberid, mjd=mjd, znum=znum, nsmooth=nsmooth, $
 ...
 _EXTRA=Extra, rm_info=rm_info, old_spec = old_spec,calibdir=calibdir
   ...
   ; populate info of the RM targets
   if keyword_Set(rm_info) then begin
      file_rm = getenv('IDLRM_DIR') + '/etc/target_fibermap.fits'
      rm_fiber = mrdfits(file_rm, 1, /silent)
      ;help, rm_fiber
   endif
   ...
   if (NOT keyword_set(mjd)) then begin
      ...
      for iplate=0, nplate-1 do begin
         mjd1 = 0
         rm_readspec, plate[iplate], mjd=mjd1, _EXTRA=Extra, /silent,calibdir=calibdir
   ...
   if (keyword_set(zwarning)) then begin
      ...
      for iplate=0L, nplate-1L do begin
         rm_readspec, plate[iplate], mjd=mjd[iplate], $
          _EXTRA=Extra, /silent, zans=zans,calibdir=calibdir
   ...
   if (NOT keyword_set(fiberid)) then begin
      rm_readspec, plate, mjd=mjd, 0*plate+1, nfiber=nfiber_tmp, $
       _EXTRA=Extra, /silent,calibdir=calibdir
...
;   endif
   ...
   while (ifiber LT nfiber) do begin
      ...
      rm_plotspec1, platelist[ifiber], fiberid[ifiber], mjd=mjdlist[ifiber], $
       znum=znum, zmanual=zmanual, nsmooth=nsmooth, zline=zline, $
       nosyn=nosyn, noerr=noerr, masksky=masksky, $
       sky=sky, ormask=ormask, andmask=andmask, psfile=psfile, $
       xrange=xrange, yrange=yrange, noerase=noerase, netimage=netimage, $
       restframe=restframe, allexp=allexp, _EXTRA=Extra, rm_info = rm_info $
       , rm_fiber = rm_fiber, old_spec = old_spec,calibdir=calibdir
         ...
         if (ifiber LT nfiber-1) then begin
            ...
            'V': begin
                    if (keyword_set(getenv('PHOTOOP_DIR'))) then begin
                       rm_readspec, platelist[ifiber], fiberid[ifiber], $
                        mjd=mjdlist[ifiber], zans=zans, _EXTRA=Extra, /silent
                       plotspec_image, ra=zans.plug_ra, dec=zans.plug_dec, $
                        cutout=300, /calibrate, /register, /allid,calibdir=calibdir
                    endif else begin
            ...
            'P': begin
                    ...
                    if (NOT keyword_set(mjd)) then $
                     rm_readspec, plate, mjd=mjd, _EXTRA=Extra, /silent,calibdir=calibdir
                    if (NOT keyword_set(mjd)) then begin
                       print, 'MJD not found for plate ', plate
                       !quiet = quiet
                       return
                    endif
                    rm_readspec, plate, mjd=mjd, nfiber=nfiber, $
                     _EXTRA=Extra, /silent,calibdir=calibdir
                    platelist = replicate(plate,nfiber)
                    mjdlist = replicate(mjd,nfiber)
                    zmanual = 0.
