...
pro rm_spfluxcorr_v5, objname, adderr=adderr, combinedir=combinedir, $
 bestexpnum=bestexpnum;, indf=indf
   ...
   ; Treat the blue and red bestexpnum separately
   
   if not keyword_set(bestexpnum) then begin
     ibest_b = (where(camcolor EQ 'b' AND expnum EQ expnum[0], nblue))[0]
     ibest_r = (where(camcolor EQ 'r' AND expnum EQ expnum[1], nred))[0]
     bestexpnum=expnum
   endif else begin
...
; ???
      ...
      if camcolor[ifile] eq 'b' then thisbestexpnum=bestexpnum[0] else $
        thisbestexpnum=bestexpnum[1]

      if (expnum[ifile] EQ thisbestexpnum) then begin
   ...
   for iobj=0L, nobj-1 do begin
     ...
     for icam=0L,1L do begin
        iiter = 0L
        while (iiter LT maxiter1) do begin
           ; Loop over exposures

           ; do it separately for the blue and red cameras
           sigvec = 0 * allflux[*,iobj,i1[icam] ]
           for iexp=0L, nexp-1 do begin
              ...
              if (explist[iexp] EQ bestexpnum[icam] ) then begin
                 ymult1 = 0
                 yadd1 = 0
              endif else begin
                 if (keyword_set(outmask)) then qgood = outmask $
                  else qgood = 1
                 thisivar = 0 * allivar[*,iobj,i1[icam]]
                 indx = where(allivar[*,iobj,i2[icam]] GT 0 $
                  AND allivar[*,iobj,i1[icam]] GT 0, ct)
                 if (ct GT 0) then $
                  thisivar[indx] = 1. / (1./(allivar[*,iobj,i1[icam]])[indx] $
                   + 1./(allivar[*,iobj,i2[icam]])[indx])
                 solve_poly_ratio, xarray[*,iobj,icam], $
                  allflux[*,iobj,i2[icam]], allflux[*,iobj,i1[icam]], thisivar*qgood, $
  ;                npoly=3, nback=2, yfit=yfit1, ymult=ymult1, yadd=yadd1
                 ...
                 sigvec1 = (yfit1 - (allflux[*,iobj,i1[icam]])[*]) * sqrt(thisivar)
                 sigvec = sigvec > abs(sigvec1)
              endelse

              if (keyword_set(ymult1)) then begin
                 ymult[*,iobj,i2[icam]] = ymult1
                 yadd[*,iobj,i2[icam]] = yadd1
              endif
     ...
     for icam=0L,1L do begin
      ...
      for iexp=0L, nexp-1 do begin
         if (explist[iexp] NE bestexpnum[icam]) then begin
            i_b = where(camcolor EQ 'b' AND expnum EQ explist[iexp], ct1)
            i_r = where(camcolor EQ 'r' AND expnum EQ explist[iexp], ct2)
            i2 = [i_b,i_r]

            ; Set default values in caes all fits are bad
            ymult[*,iobj,i2[icam]] = 1
            yadd[*,iobj,i2[icam]] = 0

            qgood =allivar[*,iobj,i2[icam]] GT 0 $
             AND allivar[*,iobj,i1[icam]] GT 0 $
             AND outmask GT 0
               ...
               while (npoly1 LT maxpoly) do begin
                  ...
                  if (ct GT 0) then $
                   thisivar[indx] = 1. / (1./(allivar[*,iobj,i1[icam]])[indx] $
                    + 1./(allivar[*,iobj,i2[icam]])[indx])
                  solve_poly_ratio, xarray[*,iobj,icam], $
                   allflux[*,iobj,i2[icam]], allflux[*,iobj,i1[icam]], $
                  ...
                  solve_poly_ratio, xarray[*,iobj,icam], $
                   allflux[*,iobj,i2[icam]], allflux[*,iobj,i1[icam]], $
                   allivar[*,iobj,i2[icam]], allivar[*,iobj,i1[icam]] * qgood, $
...
;               endif
   ...
   for ifile=0L, nfile-1 do begin
      ;if keyword_set(indf) then begin
      ;   corrfile = djs_filepath(string(camname[ifile], expnum[ifile], indf-1, $
      ;     format='("spFluxcorr-", a2, "-", i8.8, "-", i2.2, ".fits")'), $
      ;     root_dir=combinedir)
      ;endif else begin
