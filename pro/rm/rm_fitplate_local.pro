...
;   Just at rm_fitplate, but instead of calling "rm_qsofit", 
;     call "rm_local_qsofit" to perform local continuum fit
;   The purpose is to measure narrow lines such as [OII], [NeV]
;     more accurately than using the global continuum fit. 
;
; CALLING SEQUENCE:
;   rm_fitplate_local,7338,56660,zlist=zlist,calibdir='recalib/'
;   rm_fitplate_local,[7338,7339],[56660,56683]
;   rm_fitplate_local,0,56837,calibdir='wh_skysub/',outdir=outdir, $
;    emparfile=emparfile
;
; INPUTS:
;   plate	- Plate number
...
;
;-----------------------------------------------------------------------------
pro rm_fit1fiber_local,plate,fiberid,mjd,calibdir=calibdir,ntrial=ntrial,silent=silent $
   ...
   if n_elements(calibdir) eq 0 then calibdir = 'wh_skysub/'
   ...
   if n_elements(fiberid) gt 1 then begin
      ...
      for i=0L, nobj-1L do begin
         rm_fit1fiber_local,plate[i],fiberid[i],mjd[i],calibdir=calibdir,ntrial=ntrial $
   ...
   rm_local_qsofit,lam0,flux0,err0,z0,outdir=outdir,append=append $
   ...
   for jj=0,ntrial-1 do begin
      rm_local_qsofit,lam0,flux0,err0,z0,/noplot,/silent,/diet,/add_noise,para=para
      tag_rej=['LINE_FIT_ERR']
...
;-------------------------------------------------------------------------
pro rm_fitplate_local, plate,mjd,zlist=zlist,calibdir=calibdir,range=range,ntrial=ntrial, $
      silent=silent,coadd=coadd,emparfile=emparfile,outdir=outdir,add_bhmass=add_bhmass, $
      figfile=figfile,outfile=outfile,linename=linename,trim_edge=trim_edge, err_scal=err_scal
   ...
   if (size(plate))[0] ne 0 then begin
      for i=0, n_elements(plate)-1 do $
        rm_fitplate_local, plate[i],mjd[i],zlist=zlist,calibdir=calibdir,range=range,ntrial=ntrial, $
   ...
   if not keyword_set(outdir) then outdir = '/data3/yshen/spectro/bossredux/v5_7_1/0000/qsofit/wh_skysub/local_fit/'
   ...
   for i=0L, nqso - 1L do begin
      ...
      rm_local_qsofit, lam0,flux0,err0,z0,outdir=outdir,/append,objtag=objtag,silent=silent $
      ...
      if n_elements(err_array) ne 0 then tmp=temporary(err_array)
      for jj=0,ntrial-1 do begin
         rm_local_qsofit,lam0,flux0,err0,z0,/noplot,/silent,/diet,/add_noise,para=para,emparfile=emparfile
         tag_rej=['LINE_FIT_ERR']
         remove_tags, para, tag_rej, para_keep
