...
;   Same as readspec, but slightly tweaked for RM purposes
;
; CALLING SEQUENCE:
;   rm_readspec, plate, fiber, [mjd=, znum=, flux=, flerr=, invvar=, $
;    andmask=, ormask=, disp=, sky=, plugmap=, loglam=, wave=, tsobj=, $
;    zans=, zmanual=, zline=, synflux=, lineflux=, objhdr=, zhdr=, nfiber=, $
;    topdir=, path=, /align, /silent, calibdir=calibdir ]
;
; INPUTS:
...
;                calibdir='wh_skysub/' to use the recalibrated, sky-improved spec
;
; OUTPUTS:
;
; OPTIONAL OUTPUTS:
...
;------------------------------------------------------------------------------
pro rm_readspec1, plate, rownums, mjd=mjd, flux=flux, flerr=flerr, invvar=invvar, $
 ...
 align=align, silent=silent, qread=qread, sdss=sdss,calibdir=calibdir
   ...
   if keyword_set(calibdir) then begin
       filename = 'spPlate-' + platestr + '-' + mjdstr + '.fits'
       filename = lookforgzip(filepath(filename, root_dir=topdir, $
        subdirectory=[twoddir,platestr,calibdir]), count=ct)
   endif
   ...
   if (qread.zans OR qread.synflux OR qread.zhdr) then begin
      ...
      if (keyword_set(path)) then $
       zfile = lookforgzip(filepath(zfile, root_dir=path, $
        subdirectory=[oneddir]), count=ct) $
      else $
      ...
      if keyword_set(calibdir) then begin
       if (NOT keyword_set(znum)) then $
        ...
        zfile = 'spZall-' + platestr + '-' + mjdstr + '.fits'
       zfile = lookforgzip(filepath(zfile, root_dir=topdir, $
        subdirectory=[twoddir,platestr,calibdir,oneddir]), count=ct)
      endif
   ...
   if (qread.zline OR qread.lineflux) then begin
      ...
      if (keyword_set(path)) then $
       linefile = lookforgzip(filepath(linefile, root_dir=path, $
        subdirectory=[oneddir]), count=ct) $
      else $
...
;------------------------------------------------------------------------------
pro rm_readspec, plate, fiber, mjd=mjd, flux=flux, flerr=flerr, invvar=invvar, $
 andmask=andmask, ormask=ormask, disp=disp, sky=sky, plugmap=plugmap, $
...
;   endif
   ...
   if (NOT keyword_set(fiber)) then begin
      ...
      if (keyword_set(mjd)) then mjd_tmp=mjd $
       else rm_readspec, plate, plate*0+1, mjd=mjd_tmp, $
        silent=silent, _EXTRA=Extra
      rm_readspec, plate, plate*0+1, mjd=mjd_tmp, nfiber=nfiber_tmp, $
       silent=silent, _EXTRA=Extra
...
;      if (keyword_set(silent)) then print, '+', format='(A,$)'
      ...
      mjd1 = mjdnums[ifile]
      rm_readspec1, platenums[ifile], irow, mjd=mjd1, $
       ...
       align=align, silent=silent, qread=qread, _EXTRA=Extra, calibdir=calibdir
