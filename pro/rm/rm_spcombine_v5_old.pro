...
pro rm_spcombine_v5, planfile, docams=docams, adderr=adderr, xdisplay=xdisplay, $
 ...
 xyfit=xyfit, skipfcorr=skipfcorr
  ...
  if not keyword_set(topdir) then $
       topdir='/data3/quasar/yshen/spectro/bossredux/v5_6_0/7338/'

  ; Default is to do the spectrophotometry on all exposures at once
  if n_elements(oneexp) eq 0 then oneexp = 0L
  ...
  if keyword_set(skipfluxing) then logfile = 'spDiagcomb-' + platemjd + '-quick.log'
        ...
        if (ct1 GT 0) then begin
           for ido=0L, ct1/2 - 1L do $
              rm_spflux_v5, objname[i1[ido*2:ido*2+1]], adderr=adderr, $
               ...
               useairmass=useairmass,xyfit=xyfit
        endif
     endelse
