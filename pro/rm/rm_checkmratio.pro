   ...
   if keyword_set(nprox) or keyword_set(xyfit) then begin
      ...
      if keyword_set(xyfit) then begin
         ...
         for ipix=0L, npix_new - 1L do begin
            ...
            ; this is an old version, fit a B-spline on xfocal, using yfocal as the 2nd dimension
            ...
            ; this is the new version, fit a 2d polynominal
...
pro rm_checkmratio, calibname, calibdir=calibdir, topdir=topdir,title=title,nprox=nprox,$
   ...
   colors = fsc_color(['opposite','red','green','cyan','blue','magenta','gold','dark green','Olive','dark gray'])
   yrange=[0,0.3]
   ...
   if keyword_set(xyfit) then xyouts, xrange[0]+(xrange[1]-xrange[0])*0.1, 0.08*yrange[1],$
     'xyfit',charsize=1.0
...
pro rm_checkmratio_1exp, calibname, calibdir=calibdir, topdir=topdir,title=title, $
   ...
   !x.margin = [7, 14]
   plot,[0],[0],xrange=xrange,yrange=yrange,/xsty,/ysty,/nodata, $
     xtitle='Wavelength',ytitle='mratio deviation [100%]', $
     title=title
   ...
   if keyword_Set(nprox) then xyouts, xrange[0]+(xrange[1]-xrange[0])*0.7, 0.8*yrange[0],$
     'Nprox='+string(nprox,format='(i0)'),charsize=1.0,color=fsc_color('opposite',256L)
   if keyword_set(xyfit) then xyouts, xrange[0]+(xrange[1]-xrange[0])*0.7, 0.8*yrange[0],$
     'xyfit',charsize=1.0,color=fsc_color('opposite',256L)


   if not keyword_set(noanno) then begin
      xpos = 0.75 & ypos = 0.95
      xyouts, xpos, ypos, ' airmass',charsize=1.0,/norm, color=fsc_color('opposite',256L)
      isort = sort(airmass)
      items=string(airmass[isort],format='(f4.2)')
      ...
      xpos = xpos + 0.075
      xyouts, xpos, ypos, '   xfocal',charsize=1.0,/norm, color=fsc_color('opposite',256L)
      legend, items,color=colors[isort],textcolor=colors[isort],pos=[xpos,ypos],box=0,charsize=1.0,/norm
      isort = sort(yfocal)
      items=string(yfocal[isort],format='(f0.1)')
      xpos = xpos + 0.075
      xyouts, xpos, ypos, '   yfocal',charsize=1.0,/norm, color=fsc_color('opposite',256L)
      legend, items,color=colors[isort],textcolor=colors[isort],pos=[xpos,ypos],box=0,charsize=1.0,/norm
   endif
   cgloadct,0
