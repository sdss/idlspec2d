...
;   Adapted from vdispfit in idlspec2d
;
; CALLING SEQUENCE:
;   vdans = rm_vdispfit(objflux, objivar, [ objloglam, hdr=, zobj=, npoly=, $
...
;------------------------------------------------------------------------------
function rm_vdispfit, objflux, objivar, objloglam, $
 ...
 dzpix=dzpix, z_err=z_err, return_chisq=return_chisq
   ...
   if (nobj GT 1) then begin
      ...
      for iobj=0, nobj-1 do begin
         ...
         vdans1 = rm_vdispfit(objflux[*,iobj], objivar[*,iobj], $
...
;          istar+1, nstar, string(13b)
      ...
      ; Added [OIII]4364.44 (vac wave), HeII 4687.02 (vac wave)
      ; [FeVII]5160, [NI]5201, [Fe XIV]5304
