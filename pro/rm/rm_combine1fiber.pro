...
;   This is a revised version of combine1fiber for RM purposes
;
; CALLING SEQUENCE:
;   rm_combine1fiber, inloglam, objflux, [ objivar, finalmask=, indisp=, skyflux=,$
...
;   inandmask      - Input AND masks [NPIX,NSPEC]
;   inormask       - Input OR masks [NPIX,NSPEC]
...
;-
;------------------------------------------------------------------------------
pro rm_combine1fiber, inloglam, objflux, objivar, $
  ...
  if (keyword_set(inandmask)) then $
    if (n_elements(inandmask) NE npix) then $
    message, 'Dimensions of INLOGLAM and INAMDMASK do not agree'
  if (keyword_set(inormask)) then $
    if (n_elements(inormask) NE npix) then $
    message, 'Dimensions of INLOGLAM and INORMASK do not agree' 
  ...
  if (ngood EQ 0) then begin
    ...
    if (keyword_set(inandmask)) then bitval = bitval OR $
      (pixelmask_bits('NOPLUG') * (inandmask[0] AND pixelmask_bits('NOPLUG')))
    ...
    for igrp=0L, nstart-1 do begin
      ...
      if (keyword_set(sset) AND numinside GT 0) then begin
        ...
        if (ireplace[0] NE -1) then begin
          ...
          if (keyword_set(inandmask)) then $
            inandmask[ss[ireplace]] = inandmask[ss[ireplace]] OR $
            pixelmask_bits('COMBINEREJ')  
            
          if (keyword_set(inormask)) then $
            inormask[ss[ireplace]] = inormask[ss[ireplace]] OR $
            pixelmask_bits('COMBINEREJ')
    ...
    for j=0L, max(specnum) do begin
      ...
      if (these[0] NE -1) then begin
        ...
        if (inbetween[0] NE -1) then begin
          ...
          if (arg_present(andmask) AND keyword_set(inandmask)) then begin
            andmask[lowside] = andmask[lowside] AND inandmask[these]
            andmask[highside] = andmask[highside] AND inandmask[these]
          endif

          if (arg_present(ormask) AND keyword_set(inormask)) then begin
            ormask[lowside] = ormask[lowside] OR inormask[these]
            ormask[highside] = ormask[highside] OR inormask[these]
          endif
