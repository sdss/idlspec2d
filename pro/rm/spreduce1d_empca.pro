...
;   endif else chop_data = 0
   ...
   plateid = long(sxpar(hdr, 'FIELDID'))
   ...
   t0 = systime(1)
   res_qso = zrefind(objflux, objivar, eigendir=eigendir, hdr=hdr, $
    pwidth=11, pspace=1, width=11, zold=res_qso, $
    plottitle=plottitle, doplot=doplot, debug=debug)
   splog, 'CPU time to re-fit QSO redshifts = ', systime(1)-t0

   ; perform refinement of z measure
   ; mask pixels in CIV and blueward in the fits by calling a tweaked zrefind routine
   ; changed pwidth/width to be 20 pixels instead of 11 pixels
   t0 = systime(1)
   res_qso = zrefind_ys(objflux, objivar, hdr=hdr, $
    pwidth=20, pspace=1, width=20, zold=res_qso, $
    plottitle=plottitle, doplot=doplot, debug=debug)
   splog, 'CPU time to refine QSO (w/ CIV) redshifts = ', systime(1)-t0
   ...
   eigenfile = 'spEigenStar-?????.fits'  ;spEigenQSO-EMPCA-cleaned.fits
...
flambda2fnu = 0 ; Free memory
   ...
   ;----------
   ; Add other fields to the output structure; plate
...
if (0) then begin ; ???
   ...
   res_vshift = star_dvelocity(res_all[0].field, mjd=res_all[0].mjd, $
    fiberid=thisid, path='.')
