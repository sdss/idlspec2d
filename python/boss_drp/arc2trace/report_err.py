...
class Reports:
    ...
    def add(self, fitsname, cam, message, designMode):
        self.messages.apppend({'fitsname': fitsname, 'cam': cam,
                               'message': message, 'designMode':designMode})
    
    def finalize(self):
        if len(self.messages) == 0: return
        i = 0
        if plan.obs.lower() == 'apo':
            sos_data = 'BOSS_SPECTRO_DATA_N'
        else:
            sos_data = 'BOSS_SPECTRO_DATA_S'
        
        logfile = ptt.join(os.getenv('BOSS_SPECTRO_REDUX'), f'logfile-{plan.mjd}.fits')
        ...
        if lock(logfile,niter=10, pause=2):
            try:
                ...
                for mess in self.messages:
                    hdr = fits.getheader(ptt.join(os.getenv(sos_data),f'{plan.mjd}',mess['fitsname']))
                    ...
                    log.add_row([hdr['FILENAME'].encode('utf-8'),
                                 ...
                                 mess['designMode'].strip().encode('utf-8'),
                                 str(hdr['EXPOSURE']).zfill(8).strip().encode('utf-8'),
                                 mess['cam'].strip().encode('utf-8'),
        ...
        run_soslog2html(logfile, plan.mjd, plan.obs)
        return
