...
from boss_drp.utils import (find_nearest_indx, Sphdrfix, getcard)
...
def write_traceplan(allexps, mj, ftype, OBS, exclude_arc, sav_dir, topdir, 
                    run2d, release, clobber, override_manual, plan2d=False):
    thismjd = int(mj)
    if len(allexps) == 0:
    ...
    if plan2d:
        splog.info(f'Planning spTrace for {mj}')
    manual = 'F'
