...
#        print(data.to_string())
    ...
    if len(run2ds) == 1:
        ...
        if not epoch:
            outdir = ptt.join(outdir,'daily','spCalib_QA-'+run2ds[0]+'-'+obs+'.png')
        else:
            outdir = ptt.join(outdir,'epoch','spCalib_QA-'+run2ds[0]+'-'+obs+'.png')
    else:
        outdir = ptt.join(getenv("BOSS_SPECTRO_REDUX"), test_path)
        if not epoch:
            outdir = ptt.join(outdir,'spCalib_QA-'+'+'.join(run2ds)+'-'+obs+'.png')
        else:
            outdir = ptt.join(outdir,'spCalib_QA-epoch-'+'+'.join(run2ds)+'-'+obs+'.png')
