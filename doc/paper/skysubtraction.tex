...
with a common plug plate configuration.  The flat-field extraction is 
...
(as a function of wavelength) during a single setup.   
A flat-field frame is labeled rejected by the pipeline if one of the
...
reliable centroids per spectrum, are fit to produce the final wavelength
coefficients which give the logarithmic (base 10) wavelengths as a function
of spectral pixel.

An arc-lamp image is declared bad using the first 4 tests as employed
...
The fitted quantity is misnomerly (sp?) referred to as the arc-lamp dispersion,
although the values reflect the standard deviation of a Gaussian spectral
profile in pixel units.
