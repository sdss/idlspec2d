...
Top level, call frm spcalib as of r54990 (20020301):  
 - Trace320crude, then fit with 5th-order polynomial.
 - The current trunk code does not match what was used for the SDSS legacy reductions, especially trace_cen.  What revision corresponds to the one used in the DR reductions??

 - Find initial fiber positions on each flat-field frame acquired during the 
plate observation sequence.  The final structure is a 320x2048 set of column 
positions representing the best estimate of the fiber column position in each 
row of the frame.
 - Given the camera optics the column positions bow inward towards the middle 
of the CCD frame.  The fibers are more closely spaced in column position for 
rows near the middle of the CCD.  The set of 21 rows about the center of the 
CCD is chosen to find the initial fiber positions.  A median\footnote{iterative
median, see appendix} is performed at each column positions, and a single row
is analyzed representing the median value per column.  The "median" central row
is cross correlated with a template representing expected fiber positions taking
into account fiber spacings and bundle separations.
- Given the best correlation offset (and minimum correlation value of ??), 
tweak each expected position from the template to the best empirical position 
in the median row by calculating a flux-weighted mean over a window of 6 pixels.

to locate the majority of profile peaks (see Figure ??? for examples of fiber
profiles in a typical SDSS frame).  

Find approximate centers of all 320 fiber profiles for a single row, and note uncertain identifications due to bad pixels or falling off CCD frame.  The profile position is calculated as the interpolated peak in cross-sectional counts.  Cross correlate the identified positions with a template based on known fiber spacings.  
??  When did trace_cen move to parinfo standard ??

From initial centers move up and down the CCD images and "trace" the position centers by calculating the best count-weighted center over a fixed window based on the previous center.

Then perform a polynomial fit on all 320 fibers profiles as column positions versus row number.  A 4th order chebyshev?? polynomial is sufficient.  The polynomial fit is purely empirical and there is no optical model or other physical constraint on the polynomial solutions.
